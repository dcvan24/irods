...
parseGenQueryOut(
    ...
    std::vector<std::string>&   resc_indicies,
    ...
    for ( int i=0; i<genQueryOut->rowCnt; ++i ) {
        ...
        resc_indicies.push_back( std::string( t_res ) );
...
int showRescTree( const char *name, const char *zoneArgument, rcComm_t *Conn , DrawingStyle drawing_style) {
    ...
    // parse results
    int offset = 0;
    status = parseGenQueryOut( offset, genQueryOut, resc_names, resc_indicies, resc_types, resc_parents, resc_map, roots );
    ...
    status =  build_child_list( resc_names, resc_parents, resc_indicies, resc_children );
    if( status < 0 ) {
